...
inline void mouse_button_callback(GLFWwindow*, int button, int action, int)
{
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        // ── Передаём pellets и spreadDeg — для дробовика 8 пеллет с разбросом 4°,
        //    для обычного оружия pellets=1, spreadDeg=0 (нет разброса)
        doShoot(
            player.pos + glm::vec3(0, player.eyeH, 0),
            camFront,
            def.fireRate,
            def.recoilKick,
            def.pellets,       // ← новый параметр: кол-во пеллет
            def.spreadDeg      // ← новый параметр: конус разброса
        );
