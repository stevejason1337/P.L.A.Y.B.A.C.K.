// Textrenderer.cpp
// STB_TRUETYPE_IMPLEMENTATION — ровно один раз, только здесь
...
static unsigned int _compileShader(unsigned int type, const char* src)
{
    ...
    if (!ok) {
        char l[512]; glGetShaderInfoLog(sh, 512, NULL, l);
        std::cerr << "[TEXT] Shader: " << l;
    }
    return sh;
}
...
static unsigned int _linkProgram(const char* vert, const char* frag)
{
    ...
    if (!ok) {
        char l[512]; glGetProgramInfoLog(p, 512, NULL, l);
        std::cerr << "[TEXT] Link: " << l;
    }
...
// ── init ──────────────────────────────────────────────────────
bool TextRenderer::init(const std::string& path, float h, int sw, int sh)
{
    ...
    textShader = _linkProgram(TEXT_VERT, TEXT_FRAG);
    rectShader = _linkProgram(RECT_VERT, RECT_FRAG);
    ...
    // Выделяем cdata как stbtt_bakedchar, храним через void*
    auto* cd = new stbtt_bakedchar[96];
    stbtt_BakeFontBitmap(buf.data(), 0, h, bmp.data(), 512, 512, 32, 96, cd);
    cdata = cd;
...
// ── drawRect ──────────────────────────────────────────────────
void TextRenderer::drawRect(float x, float y, float w, float h, glm::vec4 col)
{
...
// ── drawText ──────────────────────────────────────────────────
float TextRenderer::drawText(const std::string& text, float x, float y, glm::vec4 col)
{
    // Каст из void* обратно в stbtt_bakedchar* — безопасно, мы сами аллоцировали
    auto* cd = static_cast<stbtt_bakedchar*>(cdata);
    ...
    for (char c : text) {
        ...
        stbtt_GetBakedQuad(cd, 512, 512, c - 32, &cx, &y, &q, 1);
        float verts[6][4] = {
...
// ── textWidth ─────────────────────────────────────────────────
float TextRenderer::textWidth(const std::string& text)
{
    auto* cd = static_cast<stbtt_bakedchar*>(cdata);
    float x = 0, y = 0;
    for (char c : text) {
        if (c < 32 || c > 127) continue;
        stbtt_aligned_quad q;
        stbtt_GetBakedQuad(cd, 512, 512, c - 32, &x, &y, &q, 1);
    }
    return x;
}